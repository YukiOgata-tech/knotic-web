    ...
    <div className="grid gap-4">
      <Card className="border-black/20 bg-white/90 dark:border-white/10 dark:bg-slate-900/80">
        ...
        <CardContent className="grid gap-3">
          {notice ? <p className="rounded-md border border-emerald-300/40 bg-emerald-100/60 px-3 py-2 text-sm text-emerald-900 dark:border-emerald-500/30 dark:bg-emerald-900/20 dark:text-emerald-200">{notice}</p> : null}
          {error ? <p className="rounded-md border border-red-300/40 bg-red-100/60 px-3 py-2 text-sm text-red-900 dark:border-red-500/30 dark:bg-red-900/20 dark:text-red-200">{error}</p> : null}

          <form className="flex flex-wrap items-end gap-2" method="get">
            <div className="min-w-[220px] flex-1">
              <label className="mb-1 block text-xs text-muted-foreground">契約者検索（slug/display_name）</label>
                ...
                <TableRow key={tenant.id}>
                  <TableCell className="whitespace-normal">
                    ...
                    <form action={startImpersonationAction} className="mt-2">
                      ...
                      <button type="submit" className="inline-flex items-center justify-center rounded-md border border-black/15 px-2 py-1 text-xs dark:border-white/20">代理閲覧を開始</button>
                    </form>
                  </TableCell>
                  <TableCell className="whitespace-normal">
                    {tenant.subscription ? (
        ...
        <Card className="border-black/20 bg-white/90 dark:border-white/10 dark:bg-slate-900/80">
          ...
          <CardContent>
            <form action={upsertContractOverrideAction} className="grid gap-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  <label className="mb-1 block text-xs text-muted-foreground">effective_from (ISO, optional)</label>
                  <Input name="effective_from" placeholder="2026-03-01T00:00:00+09:00" />
                </div>
                <div>
                  <label className="mb-1 block text-xs text-muted-foreground">effective_until (ISO, optional)</label>
