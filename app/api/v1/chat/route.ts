...
async function handleChat(request: NextRequest) {
  ...
  const instructions = [
    ...
    `## 出力形式`,
    `- 回答はMarkdownで構成`,
    `- 最初に短い要約を置き、区切りには##、必要に応じて###を使う`,
    `- 見出しや箇条書き、番号付きリストなどは、内容に合わせて自然に付ける`,
    `- セクション間には空行を入れ、箇条書きだけが長く続く回答にしない`,
    ``,
    `## 行動ルール`,
    `- 質問には必要に応じてナレッジを検索して回答する`,
    `- 単語なども検索クエリとして使用してもいい`,
    `- 前の会話で補足された言葉も新しい検索クエリとして扱う`,
    `- ナレッジに情報がない場合のみ「情報が見つかりませんでした」と伝える`,
  ].join("\n")

  // 後処理フィルター: 内部技術情報が回答に含まれていた場合に差し替える
  const BLOCKED_TERMS = ["OpenAI", "GPT-", "ChatGPT", "言語モデル", "LLM", "大規模言語", "gpt-4", "gpt-5", "file search", "gemini", "claude"]
