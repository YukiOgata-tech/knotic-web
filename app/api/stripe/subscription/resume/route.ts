...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.redirect(new URL("/console/billing?notice=cancel_resumed", request.url))
  } catch (error) {
    console.error("[stripe.subscription.resume]", error)
    return NextResponse.redirect(new URL("/console/billing?error=resume_failed", request.url))
  }
}
