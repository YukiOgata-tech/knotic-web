// Shared utilities for index-url and index-url-llm Edge Functions.
...
export function maybeSitemap(url: string) {
  try {
...
export function artifactPath(tenantId: string, sourceId: string, kind: "raw" | "text" | "llm", suffix: string) {
  return `${tenantId}/${sourceId}/${Date.now()}-${crypto.randomUUID().slice(0, 8)}-${kind}.${suffix}`
}

export async function uploadArtifact(admin: AdminClient, path: string, content: string, contentType: string) {
...
export async function upsertSourcePage(admin: AdminClient, params: {
  sourceId: string
  tenantId: string
  botId: string
  canonicalUrl: string
...
export async function syncSourceText(admin: AdminClient, params: {
  botId: string
  botPublicId: string
  sourceId: string
  sourceLabel: string
  ...
  const previousFileId = String((sourceRow as any)?.file_search_file_id ?? "") || null
  ...
  form.set("file", blob, toSafeFileName(`url-${params.sourceId}.${params.fileExtension}`))
...
export type FetchAndProcessFn = (
  admin: AdminClient,
  params: { pageUrl: string; tenantId: string; sourceId: string; botId: string },
  send: SendFn
) => Promise<PageResult | null>

export async function runIndexingPipeline(params: {
  admin: AdminClient
  jobId: string
  send: SendFn
  ...
  if (!job || !(job as any).source_id) throw new Error("Indexing job not found or source is empty.")
  ...
  const { data: claimed } = await admin
    ...
    .update({
      ...
      worker_id: workerId,
      lock_expires_at: new Date(Date.now() + lockDurationMs).toISOString(),
      error_message: null,
    })
    .eq("id", (job as any).id)
    .eq("status", "queued")
  ...
  try {
    ...
    let pagesIndexed = 0
    let lastResults: PromiseSettledResult<PageResult | null>[] = []
    ...
    for (let i = 0; i < pageUrls.length; i += FETCH_CONCURRENCY) {
      ...
      onChunkDone(pagesIndexed, pageUrls.length)
    }
...
export function createEdgeFunctionHandler(
  processJob: (admin: AdminClient, jobId: string, send: SendFn) => Promise<void>
) {
